...
#Sum colums
Total_venn_data.df <- venn_data.df %>%  summarise( total_remain_mirna_genes = sum(remain_mirna_genes)) %>% 
        bind_cols(venn_data.df %>% summarise( total_lost_mirna_genes = sum(lost_mirna_genes))) %>% 
        bind_cols(venn_data.df %>%  summarise( total_gain_mirna_genes = sum(gain_mirna_genes)))
